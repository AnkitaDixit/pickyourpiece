...
function getVisualImageSrc(url: string): string {
  if (/^https?:\/\//i.test(url)) {
...
export default function ArticleCardVisual({
  ...
  if (visualImage) {
    ...
    const imgVisualId = `article-image-${safeSlug}`;
    const labelLines = buildLabelLines(visualImage.label);
    ...
    return (
      <div className="article-card-visual article-card-visual--engagement" role="img" aria-label={`${title} illustration`}>
        ...
        >
          <defs>
            <linearGradient id={`${imgVisualId}-overlay`} x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor={COLOR.text} stopOpacity="0.02" />
              <stop offset="100%" stopColor={COLOR.text} stopOpacity="0.38" />
            </linearGradient>
          </defs>

          <image href={getVisualImageSrc(visualImage.src)} x="0" y="0" width="640" height="400" preserveAspectRatio="xMidYMid slice" />
          <rect width="640" height="400" fill={`url(#${imgVisualId}-overlay)`} />
          <rect x="22" y="22" width="596" height="356" rx="18" fill="none" stroke={COLOR.white} strokeOpacity="0.5" />

          <rect x="36" y="34" width="336" height="92" rx="14" fill={COLOR.white} fillOpacity="0.58" stroke={COLOR.white} strokeOpacity="0.65" />

          {labelLines.map((line, index) => (
            <text
              key={`${safeSlug}-img-line-${index}`}
              x="48"
              y={68 + index * 22}
              fill={COLOR.text}
              fontSize="20"
